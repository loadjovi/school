(()=>{
  ...
  function parentFeedbackCard(){
    ...
    if(!f||!m)return "";
    return `<div class="card"><h2>💛 老師鼓勵回饋</h2><div class="notice"><div style="font-size:20px;font-weight:900">${m.icon} ${esc(m.label)}</div>${f.comment?`<div style="margin-top:8px;font-size:14px;font-weight:800">${esc(f.comment)}</div>`:""}<small style="display:block;margin-top:8px;color:var(--muted)">${esc(f.teacherName||"老師")}｜${esc(f.month||"")}${f.updatedAt?`｜${esc(String(f.updatedAt).slice(0,10))}`:""}</small></div></div>`;
  }
