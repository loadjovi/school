(()=>{
  ...
  function parentMonthlyScoreCard(){
    ...
    return `<div class="parent-monthly-score">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap"><b>📊 自主練習月評比｜期末 10%</b><span class="practice-feedback-chip">${total==null?"待老師月評":total+"/10"}</span></div>
      ...
      <small style="display:block;margin-top:8px;color:var(--muted)">計分公開：有效練習占 70%，老師月評占 30%；多位授課老師取平均。${current?"本月進行中，目前為暫估分數。":"此月份為正式月評結果。"}</small>
    </div>`;
  }
