(()=>{
  ...
  function page(){
    ...
    const map=existingMap(),count=state.comprehensiveExisting?.items?.length||0;
    return `<div class="card hero"><h2>🎶 弦樂團體課（綜合課）</h2><div class="notice"><b>參與團別：</b>A團、B團、儲備團<br><b>上課時間：</b>週五 08:45–10:15<br><b>本學期 7 次：</b>9/18、10/2、10/16、10/30、11/20、11/27、12/4</div><label>上課日期</label><input id="cmpDate" type="date" value="${esc(state.comprehensiveDate)}" onchange="changeComprehensiveDate(this.value)">${dateNote()}${state.comprehensiveLoading?'<div class="notice" style="margin-top:10px">正在載入既有點名…</div>':count?`<div class="notice" style="margin-top:10px">✅ 已載入 ${count} 筆既有點名，可直接修改後重新儲存。</div>`:'<div class="notice" style="margin-top:10px">尚無此日期點名，學生預設為「出席」。</div>'}</div>${groups.map(g=>groupBlock(g,map)).join("")}<div class="card"><button class="primary" onclick="saveComprehensive()">儲存／更新綜合課點名</button><button class="secondary" style="width:100%;margin-top:10px" onclick="exportComprehensiveFollowup()">📥 匯出本次未到／請假名單</button></div>`;
  }
