(()=>{
  ...
  window.saveEnsemble=async function(){
    ...
    try{await api("/api/ensemble-attendance",{method:"POST",body:JSON.stringify({sessionDate:$("eDate").value,groupName:g,items})});toast(`✅ ${g}團合奏課點名已儲存`)}catch(e){toast("❌ "+e.message)}
  };
