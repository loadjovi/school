(()=>{
  ...
  window.ensemblePage=function(){
    ...
    const loaded=state.ensembleExistingKey===[today,g].join("|")?state.ensembleExisting:null,saved=new Map((loaded?.items||[]).map(x=>[String(x.studentId),String(x.status||"present")])),count=saved.size,missing=Math.max(students.length-count,0),recorder=loaded?.recordedBy?`<br>點名：<b>${esc(loaded.recordedBy)}</b>${loaded.recordedByRole==="admin"?"（行政協助）":""}`:"";
    const statusBox=state.ensembleLoading?'<div class="notice" style="margin-top:10px">⏳ 正在確認點名紀錄…</div>':loaded?(count?`<div class="notice" style="margin-top:10px">✅ <b>已點名</b>｜已儲存 ${count}/${students.length} 人${missing?`，⚠️ 尚有 ${missing} 人未有紀錄`:""}。${recorder}</div>`:'<div class="notice" style="margin-top:10px">⚠️ <b>尚未點名</b>｜此日期尚無儲存紀錄。</div>'):'<div class="notice" style="margin-top:10px">ℹ️ 正在確認是否已有點名紀錄。</div>';
