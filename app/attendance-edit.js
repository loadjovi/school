(()=>{
  ...
  window.changeEnsembleAttendanceDate=async function(v){
    try{await fetchEnsemble(String(v||today()));render()}catch(e){toast("❌ 無法讀取既有團體課點名："+e.message)}
  };
  ...
  function followupRows(date,type,groupName,section,students,prefix){
    const rows=[["日期","課程類型","團別","分部","學生姓名","年級","樂器","出勤狀態","點名老師"]];
    for(const s of students){
      const status=$(prefix+s.studentId)?.value||"present";
  ...
  window.exportSectionFollowup=function(){
    ...
    const rows=followupRows(state.sectionSessionDate,"分部課",groupName,section,students,"att_");
  ...
  window.exportEnsembleFollowup=function(){
    const g=currentEnsemble();if(!g){toast("尚未設定團體課");return}
    const students=state.students.filter(s=>String(s.groupName)===g);
    const rows=followupRows(state.ensembleSessionDate,"團體課",g,"四分部合班",students,"ens_");
    if(rows.length===1){toast("✅ 本次沒有請假／缺席學生");return}
    downloadCsv(`${state.ensembleSessionDate}_${g}團_團體課_未到追蹤.csv`,rows);toast(`📥 已匯出 ${rows.length-1} 位未到學生`);
  };
  ...
  sectionPage=function(){
    ...
    return `<div class="card"><h2>分部課點名</h2>${selector}<label>上課日期</label><input id="sDate" type="date" value="${esc(state.sectionSessionDate)}" onchange="changeSectionAttendanceDate(this.value)">${days.length?`<div class="notice" style="margin-top:10px"><b>${esc(groupName)}團固定分部課：</b>${days.map(d=>dayText[d]).join("、")}</div>`:""}${loaded}</div>
      <div class="card"><h2>${esc(groupName)}團｜${esc(section)}學生名單</h2>${students.length?students.map(s=>`<div class="item"><div><b>${esc(s.name)}</b><small>${esc(s.grade)}｜${esc(s.instrument)}</small></div><select id="att_${s.studentId}" class="status-select">${statusOptions(state.sectionSessionStatuses[String(s.studentId)]||"present")}</select></div>`).join(""):`<div class="notice">目前沒有符合此團別／分部的學生。</div>`}${students.length?`<button class="primary" onclick="saveSection()">儲存／更新本次點名</button><button class="secondary" style="width:100%;margin-top:10px" onclick="exportSectionFollowup()">📤 一鍵匯出未到學生名單</button>`:""}</div>`;
  };
  ...
  window.saveSection=async function(){
    ...
    const groupName=String(c.groupName||c.group||""),section=String(c.section||"");
    const students=state.students.filter(s=>String(s.groupName)===groupName&&String(s.section||"待確認")===section);
    const items=students.map(s=>{const status=$(`att_${s.studentId}`).value;return {studentId:s.studentId,status,minutes:["present","late"].includes(status)?45:0}});
    try{
  ...
  window.ensemblePage=function(){
    const groups=Array.isArray(state.me?.ensembleGroups)?state.me.ensembleGroups.filter(x=>["A","B"].includes(x)):[];
    if(!groups.length)return `<div class="card"><h2>團體課點名</h2><div class="notice">尚未設定 A／B 團團體課授課範圍。</div></div>`;
    ...
    return `<div class="card"><h2>團體課點名</h2>${selector}<label>上課日期</label><input id="eDate" type="date" value="${esc(state.ensembleSessionDate)}" onchange="changeEnsembleAttendanceDate(this.value)"><div class="notice" style="margin-top:10px">${esc(g)}團四個分部一起點名。</div>${loaded}</div>
      <div class="card"><h2>${esc(g)}團學生名單</h2>${students.length?students.map(s=>`<div class="item"><div><b>${esc(s.name)}</b><small>${esc(s.section||"待確認")}｜${esc(s.instrument)}｜${esc(s.grade)}</small></div><select id="ens_${s.studentId}" class="status-select">${statusOptions(state.ensembleSessionStatuses[String(s.studentId)]||"present")}</select></div>`).join(""):`<div class="notice">目前此團沒有學生資料。</div>`}${students.length?`<button class="primary" onclick="saveEnsemble()">儲存／更新團體課點名</button><button class="secondary" style="width:100%;margin-top:10px" onclick="exportEnsembleFollowup()">📤 一鍵匯出未到學生名單</button>`:""}</div>`;
  };

  window.saveEnsemble=async function(){
    const g=currentEnsemble();if(!g){toast("尚未設定團體課");return}
    const students=state.students.filter(s=>String(s.groupName)===g);
    const items=students.map(s=>{const status=$(`ens_${s.studentId}`).value;return {studentId:s.studentId,status,minutes:["present","late"].includes(status)?50:0}});
    try{
      ...
      await fetchEnsemble(state.ensembleSessionDate);toast(`✅ ${g}團 ${state.ensembleSessionDate} 團體課點名已更新`);render();
    }catch(e){toast("❌ "+e.message)}
  };
