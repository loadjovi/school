(()=>{
  ...
  function followupHtml(){
    ...
    if(error)body=`<div class="error" style="margin-top:10px">讀取 ${esc(state.adminOps.date)} 整日點名失敗：${esc(error)}</div>`;
      ...
      const privateEmailText={sent:"✅ 已寄送",partial:"⚠️ 部分寄送",failed:"❌ 寄送失敗",not_configured:"⚠️ Email 尚未設定",no_recipients:"⚠️ 尚無家長 Gmail",disabled:"⏸️ 後台已關閉",pending:"準備寄送",not_required:"—"};
