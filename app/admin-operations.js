(()=>{
  ...
  function followupHtml(){
    ...
    if(error)body=`<div class="error" style="margin-top:10px">讀取 ${esc(state.adminOps.date)} 整日點名失敗：${esc(error)}</div>`;
      ...
      const courseHtml=courses.length?courses.map(x=>`<div class="item" style="align-items:flex-start"><div><b>${esc(x.label)}</b><small>${x.groups?.length?"團別："+esc(x.groups.join("、"))+"｜":""}${esc(x.time||"")}<br>請假 ${x.leave||0}｜缺席 ${x.absent||0}｜遲到 ${x.late||0}</small></div><div style="text-align:right;white-space:nowrap"><b style="font-size:18px">${x.expected||0} / ${x.attended||0}</b><small>應到 / 實到<br>${x.attendanceRate==null?"—":x.attendanceRate+"%"}</small></div></div>`).join(""):`<div class="notice" style="margin-top:10px">此日期沒有依固定課表排定的團體課；若有補課或個別課，仍會依實際點名紀錄顯示。</div>`;
      body=`<div class="grid" style="margin-top:12px"><div class="kpi"><b>${scheduledExpected}</b><span>今日應到人次</span></div><div class="kpi"><b>${scheduledAttended}</b><span>今日實到人次</span></div><div class="kpi"><b>${scheduledRate==null?"—":scheduledRate+"%"}</b><span>今日出席率</span></div><div class="kpi"><b>${c.total||0}</b><span>需追蹤筆數</span></div></div><div style="margin-top:12px"><b>今日課程／團別出席</b>${courseHtml}</div><div class="notice" style="margin-top:10px">應到依當日課表與目前在團學生名單計算；實到＝出席＋遲到。同一學生同日參加分部課及合奏課時，會依課程分別計算人次。</div>`;
    }
