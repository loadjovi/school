(()=>{
  ...
  function parentBlock(s){
    ...
    if(!parents.length)return `<div class="notice" style="margin:10px 0"><b>👪 家長 Gmail 綁定</b><br><span class="muted">尚未綁定家長 Gmail</span></div>`;
  ...
  function bindingStats(){
    ...
    return {active,boundIds,total:active.length,bound:boundIds.size,unbound,parentAccounts:parentEmails.size};
  }
  ...
  function summaryHtml(){
    ...
    return `<div class="card hero"><h2>👨‍👩‍👧 家長綁定</h2>
      <div class="grid">
        ...
        <div class="kpi"><b>${x.unbound.length}</b><span>尚未綁定</span></div>
      </div>
  ...
  function detailHtml(){
    ...
    return `<div class="card"><button class="secondary" style="margin:0 0 12px" onclick="closeParentBindingDetail()">← 返回後台首頁</button><h2>👨‍👩‍👧 家長綁定管理</h2><div class="notice">在團學生 ${x.total} 人｜已綁定 ${x.bound} 人｜未綁定 ${x.unbound.length} 人</div>
