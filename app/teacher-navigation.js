(()=>{
  ...
  function teacherHomePage(){
    ...
    return `<div class="card hero"><h2>🎓 我的教學</h2><div class="notice">請選擇本次要進行的課程。授課範圍由「⚙️ 我的教學」設定，可同時擁有分部課、團體課與個別課。</div></div>
      <div class="card"><h2>教學區域</h2>${cards||'<div class="notice">目前尚未設定任何教學區域。請先到「我的教學」勾選授課範圍。</div>'}${!cards?'<button class="primary" onclick="go(\'teacherSettings\')">前往設定我的教學</button>':''}</div>`;
  }
  ...
  if(teacherAccount()&&!["section","ensemble","private","attendance","teacherSettings","help","teacherHome"].includes(state.page)){
    state.page="teacherHome";
    render();
  }
})();
