(()=>{
  ...
  function monthlyEvaluationPanel(x){
    ...
    return `<div class="monthly-score-box">
      ...
      <div class="monthly-score-formula">有效練習 ${score.practiceDays}/${score.target} 天 → <b>${score.practicePoints}/7</b>　｜　老師平均 ${avg?avg.toFixed(2)+"/5":"待評"} → <b>${score.teacherPoints==null?"—":score.teacherPoints+"/3"}</b></div>
  ...
  window.exportPracticeMonthlyScore=function(){
    ...
    const rows=[["月份","學生姓名","年級","團別","分部","樂器","練習天數","當月目標天數","練習分(7分)","老師評比平均(5級)","參與月評老師數","老師評比明細","老師分(3分)","月總分(10分)","狀態","期末20%換算"]];
    ...
    downloadCsv(`${state.practiceProgressMonth}_自主練習月總評比_期末20%.csv`,rows);toast("📥 已匯出自主練習月總評比");
  };
  ...
  function practiceProgressPage(){
    ...
    <div class="card hero"><h2>📚 自主練習${isAdmin()?'月報':'進度'}</h2>
      ...
      <div class="monthly-score-policy"><b>📊 月總評比規則｜期末升級評量占比 20%</b><small>① 練習日期占 70%（7 分）：每天有完成自主練習紀錄即計 1 天，同一天多筆紀錄仍只計 1 天；分鐘數僅供紀錄，不列入分數。練習分＝min（練習天數 ÷ 當月目標天數，1）× 7。<br>② 老師月評占 30%（3 分）：1～5 級固定量尺；多位授課老師取平均，老師分＝平均級分 ÷ 5 × 3。<br>③ 月總分＝練習分＋老師分，滿分 10 分；學期末升級評量換算為 20 分。未完成老師月評時顯示「待評」，不先以 0 分計算。當月進行中顯示暫估，歷史月份為正式月分。</small></div>
      ...
      <button class="secondary" style="width:100%;margin-top:10px" onclick="exportPracticeMonthlyScore()">📥 匯出月總評比 CSV（期末 20%）</button>${isAdmin()?'<button class="secondary" style="width:100%;margin-top:8px" onclick="exportPracticeMonthlySummary()">📥 匯出練習統計 CSV</button>':''}
    </div>
