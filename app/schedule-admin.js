(()=>{
  ...
  function scheduleAdmin(){
    ...
    return `<div class="card hero"><div class="student"><div><h2 style="margin:0">📅 課表管理</h2><div class="muted">學校課表統一管理｜家長與老師同步</div></div><span class="badge ok">${active.length} 個課程規則</span></div>
      <div class="notice" style="margin-top:12px">平常只需在這裡維護課表。臨時停課直接按「停課」，家長首頁會立即顯示，不列入缺席。</div>
      <button class="primary" onclick="downloadSacredHeartScheduleTemplate()">📥 下載聖心課表匯入範本</button>
      <button class="secondary" style="width:100%;margin-top:8px" onclick="document.getElementById('scheduleCsv').click()">📤 匯入課表 CSV</button>
  ...
  const templateRows=[
    ...
    ["ensemble","A團合奏課","A","weekly","2","2026-09-08","2026-12-29","","12:30","13:20","週二"],
    ["ensemble","B團合奏課","B","weekly","2","2026-09-08","2026-12-29","","12:30","13:20","週二"],
    ...["2026-09-18","2026-10-02","2026-10-16","2026-10-30","2026-11-20","2026-11-27","2026-12-04"].map(d=>["comprehensive","弦樂團體課（綜合課）","A,B,儲備","date","","","",d,"08:45","10:15","固定日期"])
  ];
  ...
  window.importScheduleCsv=async function(file){
    if(!file)return;try{const rows=parseCsv((await file.text()).replace(/^\uFEFF/,""));if(rows.length<2)throw new Error("CSV 沒有課程資料");const head=rows[0].map(x=>x.trim()),items=rows.slice(1).map(r=>Object.fromEntries(head.map((h,i)=>[h,(r[i]||"").trim()]))).map(x=>({...x,weekday:x.weekday===""?0:Number(x.weekday)}));if(!confirm(`準備匯入 ${items.length} 筆課表，確認繼續？`))return;await api("/api/school-schedule",{method:"POST",body:JSON.stringify({items})});toast(`✅ 已匯入 ${items.length} 筆課表`);await loadSchedule();render()}catch(e){toast("❌ "+e.message)}finally{const i=document.getElementById("scheduleCsv");if(i)i.value=""}
  };
