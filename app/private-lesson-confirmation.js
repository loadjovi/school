(()=>{
  ...
  function canChange(x){const date=String(x.lessonDate||""),start=String(x.startTime||"");return workflow(x)==="scheduled"&&(today()<date||today()===date&&start&&nowTime()<start)}
  ...
  function scheduleEditor(x,source){
    ...
    return `<details class="private-reschedule"><summary class="private-reschedule-summary"><span class="private-reschedule-icon">📅</span><span><b>改期 / 延期此堂</b><small>調整本堂日期與時間</small></span><span class="private-reschedule-arrow">⌄</span></summary><div class="private-reschedule-body"><label>新日期</label><input id="plDate_${k}" type="date" min="${esc(today())}" value="${esc(x.lessonDate)}"><div class="row2"><div><label>開始時間</label><input id="plStart_${k}" type="time" value="${esc(x.startTime)}"></div><div><label>結束時間</label><input id="plEnd_${k}" type="time" value="${esc(x.endTime)}"></div></div><label>改期原因（選填）</label><input id="plReason_${k}" maxlength="300" placeholder="例：家庭行程調整"><button class="private-reschedule-save" onclick="reschedulePrivateLesson('${esc(x.studentId)}','${esc(x.lessonId)}','${source}')">✅ 儲存改期並同步通知</button></div></details>`;
  }
  ...
  function parentLessonRow(x){
    ...
    let action="";
    if(w==="scheduled")action=canChange(x)?`${scheduleEditor(x,"parent")}${cancelButton(x,"parent")}`:`<div class="notice" style="margin-top:10px">${String(x.lessonDate)===today()?"今天是預約上課日；開始時間前仍可改期或停課，開始後請等待老師送出完課確認。":"已到或超過上課日期，等待老師更新上課結果。"}</div>`;
  ...
  function teacherHistoryHtml(){
    const rows=sortedLessons().slice(0,40);
    if(!rows.length)return `<div class="notice">目前尚無個別課預約。</div>`;
    return rows.map(x=>{
  ...
  privatePage=function(){
    ...
    for(const x of rows){
      const id=String(x.studentId),w=workflow(x);
      if(w==="completed"&&(!latestCompleted.has(id)||String(x.lessonDate)>String(latestCompleted.get(id).lessonDate)))latestCompleted.set(id,x);
      // 只要仍是 scheduled，就代表老師尚未完成該堂課，快速清單必須顯示「已約」。
      // 優先顯示今天，其次未來最近一堂；若只有逾期未完課，也保留為「已約」供老師補登完成。
      if(w==="scheduled"){
        const current=nextScheduled.get(id);
        const rank=y=>String(y.lessonDate)===d?0:String(y.lessonDate)>d?1:2;
        if(!current||rank(x)<rank(current)||(rank(x)===rank(current)&&(rank(x)===2?String(x.lessonDate)>String(current.lessonDate):String(x.lessonDate)<String(current.lessonDate)))||String(x.lessonDate)===String(current.lessonDate)&&String(x.startTime||"")<String(current.startTime||""))nextScheduled.set(id,x);
      }
    }
    // 進入個課頁時，若今天已有 scheduled 課程，直接進入本次上課模式，
    // 不要求老師再點一次「快速選擇學生」。
    const todayScheduled=rows.filter(x=>workflow(x)==="scheduled"&&String(x.lessonDate)===d).sort((a,b)=>String(a.startTime||"").localeCompare(String(b.startTime||"")));
    if(todayScheduled.length&&!state.activePrivateLessonId&&!state.suppressAutoPrivateLesson){
      const now=nowTime();
      const current=todayScheduled.find(x=>String(x.startTime||"")<=now&&now<String(x.endTime||""));
      const upcoming=todayScheduled.find(x=>String(x.startTime||"")>now);
      state.activePrivateLessonId=String((current||upcoming||todayScheduled[todayScheduled.length-1]).lessonId);
    }
  ...
  window.openPrivateLesson=function(lessonId){
    ...
    if(!x)return;
    if(workflow(x)==="scheduled"&&String(x.lessonDate)===today()){
      state.activePrivateLessonId=String(x.lessonId);state.suppressAutoPrivateLesson=false;
      const card=document.getElementById("privateBookingForm");
      if(!card)return;
      card.innerHTML=activeLessonHtml(x);
      const list=document.getElementById("privateConfirmList");if(list)list.innerHTML=teacherHistoryHtml();
      card.scrollIntoView({behavior:"smooth",block:"start"});
      setTimeout(()=>document.getElementById("activeLessonContent")?.focus(),350);
      return;
    }
