(()=>{
  ...
  state.schoolViewer=state.schoolViewer||{checked:false,allowed:false,date:localDate(),data:null,loading:false,error:"",filter:"followup"};
  ...
  window.exportSchoolFollowup=function(){
    ...
    const rows=[["日期","課程","團別","分部","學生姓名","年級","樂器","出勤狀態","點名老師"]];
    for(const x of d.items||[])rows.push([x.date,classText[x.classType]||x.classType,x.groupName,x.section,x.name,x.grade,x.instrument,statusText[x.status]||x.status,x.teacherName]);
  ...
  function filteredItems(items=[]){const f=state.schoolViewer.filter;if(f==="all")return items;if(f==="followup")return items.filter(x=>x.status==="absent"||x.status==="leave");return items.filter(x=>x.status===f)}
