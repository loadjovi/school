...
function registrationForm(){return `<div class="card"><h2>第一次登入｜綁定學生</h2><div class="notice">登入 Gmail：${esc(state.me.email)}<br>請填寫學生姓名與班級供系統比對。年級、團別與樂器一律以學校學生主檔為準，家長填寫內容不會修改學生資料。</div><label>學生姓名</label><input id="rName" autocomplete="off" placeholder="請輸入學生完整姓名"><label>學生班級</label><input id="rClass" autocomplete="off" placeholder="請依學校名單填寫，例如：301"><div class="check"><input id="rConsent" type="checkbox"><div>我確認此 Google 帳號為上述學生之家長／監護人，並同意建立家長存取權限。</div></div><button class="primary" onclick="submitRegistration()">送出綁定申請</button></div>`}
async function submitRegistration(){if(!$("rConsent").checked){toast("請先確認家長／監護人關係");return}const studentName=$("rName").value.trim(),classCode=$("rClass").value.trim();if(!studentName){toast("請填寫學生姓名");return}if(!classCode){toast("請填寫學生班級");return}const body={studentName,classCode,consent:true};try{await api("/api/student-registration",{method:"POST",body:JSON.stringify(body)});toast("✅ 已送出，等待管理員確認");state.page="home";render()}catch(e){toast("❌ "+e.message)}}
