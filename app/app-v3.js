...
function registrationForm(){return `<div class="card"><h2>第一次登入｜學生資料登記</h2><div class="notice">登入 Gmail：${esc(state.me.email)}<br>送出後由管理員確認；核准後 Gmail 才會正式綁定學生。</div><label>家長姓名</label><input id="rParent" value="${esc(state.me.displayName||"")}"><label>與學生關係</label><select id="rRel"><option>父親</option><option>母親</option><option>監護人</option><option>其他</option></select><label>學生姓名</label><input id="rName"><div class="row2"><div><label>年級</label><select id="rGrade">${opts(grades,"三年級")}</select></div><div><label>團別</label><select id="rGroup">${opts(groups,"B")}</select></div></div><label>樂器類別</label><select id="rInst">${opts(instruments,"小提琴")}</select><label>學年度（選填）</label><input id="rYear" placeholder="例：115"><div class="check"><input id="rConsent" type="checkbox"><div>我同意此資料僅用於弦樂團學生身分、點名、練習與考核管理。</div></div><button class="primary" onclick="submitRegistration()">送出學生資料</button></div>`}
async function submitRegistration(){if(!$("rConsent").checked){toast("請先勾選資料使用確認");return}const body={parentName:$("rParent").value,relationship:$("rRel").value,studentName:$("rName").value,grade:$("rGrade").value,groupName:$("rGroup").value,instrument:$("rInst").value,schoolYear:$("rYear").value,consent:true};try{await api("/api/student-registration",{method:"POST",body:JSON.stringify(body)});toast("✅ 已送出，等待管理員確認");state.page="home";render()}catch(e){toast("❌ "+e.message)}}
...
function pendingCard(r){return `<div class="card"><h2>${esc(r.studentName)}｜待審核</h2><div class="notice">家長：${esc(r.parentName)}（${esc(r.relationship)}）<br>${esc(r.parentEmail)}</div><label>學生姓名</label><input id="rn_${r.registrationId}" value="${esc(r.studentName)}"><div class="row2"><div><label>年級</label><select id="rg_${r.registrationId}">${opts(grades,r.grade)}</select></div><div><label>團別</label><select id="rgrp_${r.registrationId}">${opts(groups,r.groupName)}</select></div></div><label>樂器</label><select id="ri_${r.registrationId}">${opts(instruments,r.instrument)}</select><label>學年度</label><input id="ry_${r.registrationId}" value="${esc(r.schoolYear||"")}"><label>對應既有學生（已有名單時使用）</label><select id="rm_${r.registrationId}"><option value="">建立新學生主檔</option>${state.master.map(s=>`<option value="${esc(s.studentId)}">${esc(s.name)}｜${esc(s.grade)}｜${esc(s.groupName)}團｜${esc(s.instrument)}</option>`).join("")}</select><div class="row2"><button class="primary" onclick="approveReg('${r.registrationId}')">核准並綁定</button><button class="secondary" style="margin-top:14px" onclick="rejectReg('${r.registrationId}')">退回</button></div></div>`}
async function approveReg(id){const body={registrationId:id,action:"approve",studentName:$(`rn_${id}`).value,grade:$(`rg_${id}`).value,groupName:$(`rgrp_${id}`).value,instrument:$(`ri_${id}`).value,schoolYear:$(`ry_${id}`).value,studentId:$(`rm_${id}`).value};try{await api("/api/student-registration",{method:"PATCH",body:JSON.stringify(body)});toast("✅ 已核准並綁定");await loadAdmin();render()}catch(e){toast("❌ "+e.message)}}
