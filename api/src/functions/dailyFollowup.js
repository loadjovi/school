...
async function canonicalId(id,students,cache){
  ...
  try{
    const a=await getStudentAliasInfo(key);
    const c=String(a.canonicalStudentId||key);
    cache.set(key,c);return c;
  }catch{
    cache.set(key,key);return key;
  }
}
...
async function collect(key,date){
  ...
  for await (const e of table(key).listEntities({queryOptions:{filter:`eventDate eq '${date.replaceAll("'","''")}'`}})){
    ...
    const row={
      rawStudentId,eventDate:String(e.eventDate||date),classType:key==="ensemble"?"ensemble":"section",
...
app.http("dailyFollowup",{
  ...
  handler:async(request)=>{
    ...
    const [sectionRows,ensembleRows]=await Promise.all([collect("section",date),collect("ensemble",date)]);
    const raw=[...sectionRows,...ensembleRows].filter(x=>["leave","absent"].includes(x.status));

    const latest=new Map();
    for(const r of raw){
      const studentId=await canonicalId(r.rawStudentId,students,canonicalCache);
