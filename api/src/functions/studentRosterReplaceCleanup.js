...
async function deactivateParentMappings(studentId,accessEmail,schoolId){
  const client=table("tenantUserStudentMap"),rows=[],sid=tenantSchoolPartition(schoolId).replaceAll("'","''");
  for await (const e of client.listEntities({queryOptions:{filter:`schoolId eq '${sid}' and RowKey eq '${String(studentId).replaceAll("'","''")}'`}}))rows.push(e);
...
async function removeTeacherPrivateAssignments(studentId,schoolId){
  const client=table("tenantTeacherProfile"),sid=tenantSchoolPartition(schoolId).replaceAll("'","''");let count=0;
...
app.http("studentRosterReplaceCleanup",{
  ...
  handler:async(request)=>{
    ...
    const keep=new Set(keepIds),term=semesterKey(schoolYear,semester),existing=await listStudentMaster("",schoolId);
    ...
    const termRows=[];
    const tenantTerm=tenantTermPartition(schoolId,schoolYear,semester);
    for await (const e of table("tenantSemesterEnrollment").listEntities({queryOptions:{filter:`PartitionKey eq '${tenantTerm.replaceAll("'","''")}'`}}))termRows.push(e);
    ...
    for(const old of termRemove){try{await table("tenantSemesterEnrollment").deleteEntity(String(old.partitionKey),String(old.rowKey))}catch(e){if(e.statusCode!==404)throw e}}
    ...
    for(const old of activeNotInTerm){
      ...
      await table("tenantStudentMaster").updateEntity(next,"Merge");
      await table("tenantStudentHistory").createEntity({partitionKey:tenantStudentPartition(schoolId,id),rowKey:rowKey("hist"),schoolId,studentId:id,changeType:"semester_not_enrolled",schoolYear,semester,changedAt:now,changedBy:access.email,oldValue:JSON.stringify(view(old)),newValue:JSON.stringify({status:"inactive",inactiveForTerm:term,historyPreserved:true})});
      results.push({studentId:id,name:old.studentName||"",action:"inactive_for_term"});
    }

    for(const old of legacyRemove){
      const id=String(old.rowKey),parentMappings=await deactivateParentMappings(id,access.email,schoolId),teacherProfiles=await removeTeacherPrivateAssignments(id,schoolId);
      await table("tenantStudentHistory").createEntity({partitionKey:tenantStudentPartition(schoolId,id),rowKey:rowKey("hist"),schoolId,studentId:id,changeType:"legacy_student_id_removed",schoolYear,semester,changedAt:now,changedBy:access.email,oldValue:JSON.stringify(view(old)),newValue:JSON.stringify({removedFromStudentMaster:true,historyPreserved:true,parentMappingsDeactivated:parentMappings,teacherProfilesUpdated:teacherProfiles})});
      try{await table("tenantStudentMaster").deleteEntity(String(old.partitionKey||tenantSchoolPartition(schoolId)),id)}catch(e){if(e.statusCode!==404)throw e}
