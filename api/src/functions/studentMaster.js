...
app.http("studentMaster",{
  ...
  handler:async(request)=>{
    ...
    const err=validate({studentName,grade,groupName,instrument,status});if(err)return json({error:err},400);
    ...
    if(request.method==="POST"){
      ...
      const entity={partitionKey:"STUDENT",rowKey:studentId,studentName,grade,groupName,instrument,schoolYear,status,createdAt:now,updatedAt:now,updatedBy:access.email};
    ...
    const entity={...old,studentName,grade,groupName,instrument,schoolYear,status,updatedAt:now,updatedBy:access.email};
