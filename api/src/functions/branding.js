...
const defaults=()=>({siteName:"聖心小學弦樂團",schoolName:"輔大聖心國小",loginSubtitle:"家長、老師與管理員共用入口",logoAlt:"聖心 Logo",primaryColor:"#245C49",logoBlobName:"",logoContentType:"image/webp",updatedAt:"",updatedBy:""});
...
async function getBranding(create=true){
  const client=await settingsClient();
  try{
    const e=await client.getEntity("SYSTEM","BRANDING");
    return {...defaults(),siteName:String(e.siteName||defaults().siteName),schoolName:String(e.schoolName||defaults().schoolName),loginSubtitle:String(e.loginSubtitle||defaults().loginSubtitle),logoAlt:String(e.logoAlt||defaults().logoAlt),primaryColor:String(e.primaryColor||defaults().primaryColor),logoBlobName:String(e.logoBlobName||""),logoContentType:String(e.logoContentType||"image/webp"),updatedAt:String(e.updatedAt||""),updatedBy:String(e.updatedBy||"")};
  }catch(e){
    if(e.statusCode!==404)throw e;
    const d=defaults();
    if(create){await client.upsertEntity({partitionKey:"SYSTEM",rowKey:"BRANDING",...d,createdAt:new Date().toISOString()},"Merge")}
    return d;
  }
}
async function saveBranding(input,updatedBy){
  const current=await getBranding(true);
  const siteName=String(input.siteName??current.siteName).trim().slice(0,80)||defaults().siteName;
  const schoolName=String(input.schoolName??current.schoolName).trim().slice(0,120);
  ...
  if(!/^#[0-9A-Fa-f]{6}$/.test(primaryColor))primaryColor=current.primaryColor||defaults().primaryColor;
  const now=new Date().toISOString();
  const entity={partitionKey:"SYSTEM",rowKey:"BRANDING",siteName,schoolName,loginSubtitle,logoAlt,primaryColor,logoBlobName:current.logoBlobName||"",logoContentType:current.logoContentType||"image/webp",updatedAt:now,updatedBy:String(updatedBy||"").slice(0,160)};
...
async function uploadLogo(dataUrl,updatedBy){
  ...
  const blob=container.getBlockBlobClient("school-logo");
  await blob.uploadData(buf,{blobHTTPHeaders:{blobContentType:contentType}});
  const client=await settingsClient(),current=await getBranding(true),now=new Date().toISOString();
  await client.upsertEntity({partitionKey:"SYSTEM",rowKey:"BRANDING",...current,logoBlobName:"school-logo",logoContentType:contentType,updatedAt:now,updatedBy:String(updatedBy||"").slice(0,160)},"Merge");
...
async function readLogo(){
  ...
  if(branding.logoBlobName){
    try{
      const container=await blobContainer();
      const blob=container.getBlobClient(branding.logoBlobName);
      const response=await blob.download();
      const chunks=[];for await(const chunk of response.readableStreamBody)chunks.push(Buffer.from(chunk));
  ...
  return {buffer:Buffer.from(DEFAULT_LOGO_WEBP_BASE64,"base64"),contentType:"image/webp",updatedAt:"default"};
}
async function deleteLogo(updatedBy){
  const branding=await getBranding(true);
  ...
  const client=await settingsClient(),now=new Date().toISOString();
  await client.upsertEntity({partitionKey:"SYSTEM",rowKey:"BRANDING",...branding,logoBlobName:"",logoContentType:"image/webp",updatedAt:now,updatedBy:String(updatedBy||"").slice(0,160)},"Merge");
  return {ok:true,updatedAt:now};
}
function publicView(b){return {siteName:b.siteName,schoolName:b.schoolName,loginSubtitle:b.loginSubtitle,logoAlt:b.logoAlt,primaryColor:b.primaryColor,logoUrl:`/api/branding-logo?v=${encodeURIComponent(b.updatedAt||"default")}`,updatedAt:b.updatedAt||""}}

app.http("branding",{methods:["GET","PATCH"],authLevel:"anonymous",route:"branding",handler:async request=>{
  if(request.method==="GET")return json(publicView(await getBranding(true)));
  ...
  return json(publicView(await saveBranding(body||{},access.email)));
}});

app.http("brandingLogo",{methods:["GET","POST","DELETE"],authLevel:"anonymous",route:"branding-logo",handler:async request=>{
  if(request.method==="GET"){
    const x=await readLogo();return {status:200,headers:{"Content-Type":x.contentType,"Cache-Control":"public, max-age=300"},body:x.buffer};
  }
  const access=await getAccess(request);if(!access.authenticated)return json({error:"Unauthorized"},401);if(access.role!=="admin")return json({error:"Forbidden"},403);
  if(request.method==="DELETE")return json(await deleteLogo(access.email));
  let body;try{body=await request.json()}catch{return json({error:"JSON 格式不正確"},400)}
