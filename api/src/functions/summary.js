...
app.http("summary",{methods:["GET"],authLevel:"anonymous",route:"summary",handler:async(request)=>{
  ...
  const [p,s,i]=await Promise.all([
    listByStudent("practice",studentId,start,end),
    listByStudent("section",studentId,start,end),
    listByStudent("privateLesson",studentId,start,end)
  ]);
  const qualifiedDays=new Set(p.filter(x=>x.qualified===true).map(x=>x.eventDate)).size;
  ...
  return json({
    ...
    sectionPresent,sectionTotal:sectionEffective.length,privatePresent,privateTotal:privateEffective.length,
    weightedScore:null
  });
}});
