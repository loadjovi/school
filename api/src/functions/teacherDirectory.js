  ...
  handler:async(request)=>{
    ...
    if(request.method==="GET"){
      ...
      for(const t of await listTeacherDirectory()){
        ...
        items.push({email:t.rowKey,teacherName:t.teacherName||"",status:t.status||"active",updatedAt:t.updatedAt||null,lastLoginAt:t.lastLoginAt||null,sectionAssignments:p?parse(p.sectionAssignments,[]):[],ensembleGroups:p?parse(p.ensembleGroups,[]):[],comprehensiveEnabled:p?.comprehensiveEnabled===true,privateStudentIds:p?parse(p.privateStudentIds,[]):[]});
      }
      return json({items});
    }
